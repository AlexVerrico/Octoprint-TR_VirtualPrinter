...
class TR_VirtualPrinter(object):
    ...
    override_temp_regex = re.compile(r"override_temp (.*)")
    rm_override_temp_regex = re.compile(r"rm_override_temp (.*)")
    ...
    def __init__(
        ...
        self._action_hooks = plugin_manager().get_hooks(
            "octoprint.plugin.TR_VirtualPrinter.custom_action"
        )
        ...
        readThread = threading.Thread(
            ...
            name="octoprint.plugins.TR_VirtualPrinter.wait_thread",
        )
        readThread.start()

        bufferThread = threading.Thread(
            target=self._processBuffer,
            name="octoprint.plugins.TR_VirtualPrinter.buffer_thread",
        )
        bufferThread.start()
    ...
    def _processIncoming(self):
        ...
        while self.incoming is not None and not self._killed:
            ...
            if data.strip() == "version":
                ...
                self._send("OctoPrint TR_VirtualPrinter v" + __version__)
                continue
            ...
            command_match = TR_VirtualPrinter.command_regex.match(data)
    ...
    def _debugTrigger(self, data):
        ...
        if data == "" or data == "help" or data == "?":
            ...
            try:
                sleep_match = TR_VirtualPrinter.sleep_regex.match(data)
                sleep_after_match = TR_VirtualPrinter.sleep_after_regex.match(data)
                sleep_after_next_match = TR_VirtualPrinter.sleep_after_next_regex.match(data)
                custom_action_match = TR_VirtualPrinter.custom_action_regex.match(data)
                prepare_ok_match = TR_VirtualPrinter.prepare_ok_regex.match(data)
                send_match = TR_VirtualPrinter.send_regex.match(data)
                set_ambient_match = TR_VirtualPrinter.set_ambient_regex.match(data)
                start_sd_match = TR_VirtualPrinter.start_sd_regex.match(data)
                select_sd_match = TR_VirtualPrinter.select_sd_regex.match(data)
                resend_ratio_match = TR_VirtualPrinter.resend_ratio_regex.match(data)

                ## BEGIN MODIFIED SECTION ## ---------------------------------------------
                override_temp = TR_VirtualPrinter.override_temp_regex.match(data)
                rm_override_temp = TR_VirtualPrinter.rm_override_temp_regex.match(data)
                ## END MODIFIED SECTION ## -----------------------------------------------
                ...
                ## BEGIN MODIFIED SECTION ## ---------------------------------------------
                # !!DEBUG:override_temp T0 50
                elif override_temp is not None:
                    # self._logger.debug("override_temp_regex match = {}".format(override_temp.group(1)))
                    cmd_data = str(override_temp.group(1)).split(" ")
                    # self._logger.debug("cmd_data = {}".format(cmd_data))
                    self._do_override[str(cmd_data[0])] = float(cmd_data[1])
                    del cmd_data

                elif rm_override_temp is not None:
                    cmd_data = str(rm_override_temp.group(1))
                    try:
                        del self._do_override[cmd_data]
                    except KeyError:
                        pass
                    del cmd_data
                ## END MODIFIED SECTION ## -----------------------------------------------
    ...
    def _generateTemperatureOutput(self):
        ...
        ## BEGIN MODIFIED SECTION ## -----------------------------------------------------
        # OrderedDict([('T', (current: float, set: float))])
        self._logger.debug("ORIGINAL TEMPS: {}".format(temps))
        if self._do_override:
            to_delete = []
            for override_heater, override_temp in self._do_override.items():
                try:
                    if temps[override_heater]:
                        temps[override_heater] = (float(override_temp), temps[override_heater][1])
                        self._logger.debug("Heater {h} is overidden to {t}"
                                           .format(h=override_heater, t=override_temp))
                except KeyError:
                    to_delete.append(override_heater)
            for i in to_delete:
                del self._do_override[i]
            del to_delete
            self._logger.debug("NEW TEMPS: {}".format(temps))
        ## END MODIFIED SECTION ## -------------------------------------------------------
